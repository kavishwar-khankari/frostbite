...
Always delegate web searches to the `websearch` subagent via the Task tool. Example: "search the web for X and return the results". The `websearch` subagent uses a cheap DeepSeek model with SearXNG MCP tools for general web research and Reddit MCP tools for Reddit-specific research — this avoids the built-in WebSearch/WebFetch cost. Never use `WebSearch` or `WebFetch` directly; use `Task(subagent_type="websearch", ...)` instead.
